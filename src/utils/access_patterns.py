...
def sequential_8_interpolation(array, cube_shape, low=4, high=60, size=50):
    """
    Sequential access and Lagrangian 8-interpolation of array. Points
        picked sequentially between [low, high], forming an 8x8x8 cube
    :param array: array to be interpolated
    :param cube_shape: cube dimensions from within to pick points
    :param low: lower bound of cube e.g. 0 will be 0,0,0, meaning first
        chunk of array. Needs to be >= 4 for 8-interpolation
    :param high: upper bound of cube e.g. 60 will be 60,60,60, meaning
        bottom-right chunk of array. Needs to be <= len(array)
            - 4 for 8-interpolation
...
def index_8_interpolation(array, rand_indices):
    ...
    # Can't generate random indices this here bcs. it penalizes
    #   Single-variable experiments (ran 3 times vs. once for joint)
...
def access_1_velocity_from_joint(xarray_arr, rand_indices):
    """
    :param xarray_arr: array to access from. Efficiently slice zarr using
        store = xarray.open_zarr(path), then use
        slice_3d = store.isel({'velocity component (xyz)': 0})
    :param rand_indices: list or array of indices to interpolate around in the given array
    """
